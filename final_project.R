rm(list = ls())#rm() is basically 'remove{base}', it is used to Remove all the Objects from a Specified Environment.


setwd("C:\\Users\\MRUNMAI VINOD BHOLE\\Desktop\\Cab-fare-prdiction-master")
...
install.packages(p)
lapply(p, require, character.only = TRUE)  #Load multiple packages at the same time in R
...
cab_train = read.csv("C:\\Users\\MRUNMAI VINOD BHOLE\\Desktop\\Cab-fare-prdiction-master\\train_cab.csv")
test = read.csv("C:\\Users\\MRUNMAI VINOD BHOLE\\Desktop\\Cab-fare-prdiction-master\\test.csv")
cab_train

sapply(cab_train,function(x)sum(is.na(x)))
#To calculate the number of NAs in the entire data.frame per column


###################  Exploratory data analysis ###########################

#Getting the number of variables and obervation in the datasets
dim(cab_train) #dimension of cab_train
dim(test) #test
#Structure of data
str(cab_train)
str(test)
...
#freqency per value eg. 0 is 57 times in col pass count


nrow(cab_train[which(cab_train$passenger_count > 6),]) #number of rows in the table having col value greater than 6.
...
cab_train= knnImputation(cab_train$fare_amount,3) 
...
cab_train$Date = as.Date(cab_train$pickup_datetime,"%Y-%m-%d")
